...
import id1212.project.error.ErrorException;
 ...
 */
	public List<Dog> findAllDogsByOwner(int ownerId) {
		
		List<Dog> dogs = dogRep.findDogByOwner_OwnerId(ownerId);
		return dogs;
	}
	...
	public Dog updateDog(int id, Dog newDog) {
		Optional<Owner> owner = ownerRep.findById(newDog.getOwner().getOwnerId());
		if(owner.isEmpty()){		
			throw new ErrorException("Dog cannot be owned by an unkonwn Owner with id: ", newDog.getOwner().getOwnerId());
		}
