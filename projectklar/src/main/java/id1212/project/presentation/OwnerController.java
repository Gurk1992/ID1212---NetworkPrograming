...
public class OwnerController {
	...
	@Autowired 
	DogResrouceAssembler dogAssembler;
	@Autowired
	private DogService service;

	@GetMapping("/owners")
	CollectionModel<EntityModel<Owner>> allOwners(){
		List<EntityModel<Owner>> owners = service.findAllOwners().stream().map(ownerAssembler::toModel).collect(Collectors.toList());
		 return new CollectionModel<>(owners, linkTo(methodOn(OwnerController.class).allOwners()).withSelfRel());
	}
